Jit.AM.defindPage({
    ...
    initEvent: function() {
        ...
        self.elements.topTabList.delegate('a', 'click', function() {
            var el = $(this);
            self.bindDataList(el.data('val'));
        });


    },
    bindDataList: function(eventTypeID) {
        var self = this;
        eventTypeID='';
        debugger;
        $.ajax({
            ...
            data: {
                ...
                list:[{type:"a",name:"b"},{type:"a",name:"b"}]
            },
            success:function(data){
        ...
        self.ajax({
            ...
            success: function(data) {
                ...
                if (data && data.code == 200 && data.content&&data.content.ItemList.length > 0) {
                    self.elements.actList.html(GetActListHtml(data.content.ItemList));
                    var $div=$(".itemAnimateAgo");
                    //动画循环开始
                    $.each($div,function(i,n){
                        function a(b){
                            $(n).addClass("itemAnimate");
                           clearTimeout(theTime);
                        }
                        var time = i*500;
                        var theTime=setTimeout(a,time);
                    });
                }
            }
        });
...
function GetActListHtml(datas) {
    ...
    for (var i = 0; i < datas.length; i++) {
        ...
        actListHtml.appendFormat("<a class=\"{1}\" href=\"javascript:Jit.AM.toPage('ActDetail','&newsId={0}')\"><div class='itemAnimateAgo'>", actItem.ActivityID, endDay <= 0 ? '' : beginDay <= 0 ? 'on' : 'ho');
