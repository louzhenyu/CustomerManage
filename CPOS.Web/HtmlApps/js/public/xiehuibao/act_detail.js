Jit.AM.defindPage({
	...
	LoadUrlValues: function() { //加载用户缓存信息
		var self = this,
			urlParams = self.getUrlParam();
		if (urlParams) {
			for (var key in urlParams) {
				var element = $("[name=" + key + "]", self.elements.actBoxInfo);
				if (element.size()) {
			...
			for (var p = 0; p < blockItem.Control.length; p++) {
				...
				switch (controlType) {
					case 1:
						...
						htmlList.appendFormat("<span class=\"wrapInput\">{2}<input id=\"{0}\" maxlength=\"128\"  type=\"{3}\" value=\"{1}\" name=\"{4}\"  ></span>", controlItem.ControlID, controlItem.Value ? controlItem.Value : '', controlItem.IsMustDo ? hasItem : '：', GetInputType(controlItem.AuthType), controlItem.ColumnName);
						htmlList.append("</p>");
					...
					case 9:
						...
						htmlList.appendFormat("<div class=\"commonArea\"><textarea id=\"{0}\" maxlength=\"500\"  name=\"{2}\" >{1}</textarea></div>", controlItem.ControlID, controlItem.Value ? controlItem.Value : '', controlItem.ColumnName);
						htmlList.append("</div>");
						break;
					case 10:
						htmlList.append("<p class=\"commonList\">");
						htmlList.appendFormat("<em class=\"commonTit\">{0}</em>", controlItem.ColumnDesc);
						htmlList.appendFormat("<span class=\"wrapInput\">{2}<input id=\"{0}\" maxlength=\"128\"  type=\"password\" value=\"{1}\" name=\"{3}\" ></span>", controlItem.ControlID, controlItem.Value ? controlItem.Value : '', controlItem.IsMustDo ? hasItem : '：', controlItem.ColumnName);
						htmlList.append("</p>");
						htmlList.append("<p class=\"comline\"></p>");
						break;
				}
