Jit.AM.defindPage({
	...
	initLoad: function() {
		...
		self.elements.authtips = $('.authtips');
	...
	upFiles: function(e) { //普通上传图片，适用与安卓系统
		...
		var self = this,
			curElement = $(e.target),
			file = curElement[0],
			controId = curElement.attr('id'),
			fu = new QuickUpload(file, {
				action: '/Project/CEIBS/CEIBSHandler.ashx?action=FileUpload',
				onFinish: function(iframe) {
					UIBase.loading.hide();
					try { //处理返回信息(需要后台配合)
						var result = eval("(" + iframe.contentWindow.document.body.innerHTML + ")"),
							curImg = $('#img' + curElement.attr(self.keyUp.vid))[0];
						if (result.code == '200') {
							curImg.src = result.content;
							curElement.attr(self.keyUp.vsrc, result.content);
						} else {
							self.tips(result.content);
						}
					} catch (ex) { //获取数据出错
						self.tips("服务器繁忙，请稍后。");
					}
		...
		if (!reg.test(thefile)) {
			self.tips("请选择图片jpg,png,bmp,gif!");
			return false;
		}
		fu.upload();
	},
...
Jit.UI.Loading(1);
		var me = JitPage,
			element = $(e.target),
			controId = element.attr(me.keyUp.vid);
		...
		for (var i = 0, f; f = files[i]; i++) {
			...
			reader.onload = (function(file) {
				...
				var fileSize = (file.size / 1024).toFixed(2) + "K",
					...
					fileType = file.type;
				return function(e) {
					var curImg = $('#img' + element.attr(me.keyUp.vid))[0];
	...
	GetControlInfos: function(pageNumber) {
		...
		$('input,select,textarea', curPages).each(function() {
			...
			if (item.attr(self.keyUp.vid)) {
				controlInfo.ControlId = item.attr(self.keyUp.vid);
			};
			var dataControlInfo = self.GetControlItem(pageNumber, controlInfo.ControlId);
			//文本类型不同，获取方式不同
			...
			for (var p = 0; p < blockItem.Control.length; p++) {
				...
				switch (controlType) {
					case 1:
						...
						break;
					case 6:
						htmlList.append("<p class=\"commonList\"style=\"display:block;\">  ");
						...
						htmlList.append("<p class=\"comline\" style=\"margin-left:0px\" ></p>");
						break;
					case 7:
					...
					case 10:
						...
						break;
					case 11:
						htmlList.appendFormat("<form id=\"form{0}\" name=\"form{0}\" action=\"/Project/CEIBS/CEIBSHandler.ashx?action=FileUpload\"  method=\"POST\" enctype=\"multipart/form-data\"    target=\"fileUp\">", controlItem.ControlID);
						htmlList.append("<p class=\"commonList file\">");
						htmlList.appendFormat("<span class=\"wrapInput\">{2}<input id=\"fileUp\" name=\"fileUp\" vid=\"{0}\"  maxlength=\"128\"   type=\"file\" value=\"{1}\"  vsrc=\"{1}\" ></span>", controlItem.ControlID, controlItem.Value ? controlItem.Value : '', controlItem.IsMustDo ? hasItem : '');
						htmlList.appendFormat("<em class=\"commonTit\">{0}</em>", controlItem.ColumnDesc);
						htmlList.appendFormat("<img src=\"{1}\" id=\"img{0}\" onerror=\"javascript:this.src='../../../images/special/europe/embaupfile.jpg';\"   vsrc=\"{1}\"  />", controlItem.ControlID, controlItem.Value || '../../../images/special/europe/embaupfile.jpg');
						htmlList.append("</p>");
