Jit.AM.defindPage({
    ...
    initEvent: function() {
        var self = this;
        this.ele.housesList.delegate(".jsListItem",this.eventType,function(){
        	var $this = $(this);
        	JitPage.toPageWithParam("HousesDetail","detailId="+$this.data("detailid")+"&orderNo="+$this.data("orderno")+"&payId="+$this.data("payid"));
        });
    },
    loadData:function(){
    	this.GetHouses();
	            ...
	            success: function(data) {
	                if(data.IsSuccess){
	                	...
	                	if (data.Data.HouseList && data.Data.HouseList.length > 0) {
		                	...
		                	self.nomoreData = true;
		                	if(self.page.pageIndex==1){ 
		                		housesList.html('<div class="noData"><img src="../../../images/special/worldunion/noData.png" alt="" /><a href="javascript:JitPage.toPageWithParam(\'HousesList\');" class="commonBtn seeHousesBtn">查看可预订楼盘</a></div>');
		                	}
	                	...
	                	if(self.page.pageIndex==1){
		            		housesList.html('<div style="text-align:center;">'+data.Message+'</div>');
		            	}
	                }                
	            },
	            complete:function(){
    ...
    renderHouseList:function(itemlists){
    	for(var i=0;i<itemlists.length;i++){
			switch (parseInt(itemlists[i].HouseState)) {
		        case 1:
		            itemlists[i].HouseStateDes = "已开盘";
		            break;
		        case 2:
		            itemlists[i].HouseStateDes = "在建中";
		            break;
		        default:
		            itemlists[i].HouseStateDes = "未知HoseState";
		            break;
		    }
