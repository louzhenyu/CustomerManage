Jit.AM.defindPage({
    ...
    upFile: function(singleImg, curElement) {
        ...
        if (xhr.upload) {
            ...
            // 开始上传
            xhr.open("POST", str + "/Project/CEIBS/CEIBSHandler.ashx?action=FileUpload", true);
            xhr.send(formdata);
            ...
            for (var p = 0; p < blockItem.Control.length; p++) {
                ...
                switch (controlType) {
                    ...
                    case 11:
                        ...
                        htmlList.appendFormat("<span class=\"wrapInput\">{2}<input id=\"{0}\" maxlength=\"128\"   type=\"file\" value=\"{1}\"  ></span>", controlItem.ControlID, controlItem.Value ? controlItem.Value : '', controlItem.IsMustDo ? hasItem : '');
                        htmlList.appendFormat("<em class=\"commonTit\">{0}</em>", controlItem.ColumnDesc);
                        htmlList.appendFormat("<img src=\"../../../images/special/europe/embaupfile.jpg\" id=\"img{0}\" />", controlItem.ControlID);
                        htmlList.append("</p>");
