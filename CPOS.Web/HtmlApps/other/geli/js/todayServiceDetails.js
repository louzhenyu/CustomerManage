define(['jquery', 'template', 'tools'], function () {
    var page = {
        	...
        	this.ajax({
                ...
                success: function (data) {
                    if (data.IsSuccess) {
                        ...
                        self.dataList = orderObj.DeviceList;
                        //保存全局变量
        ...
        initEvent: function () {
            ...
            this.ele.section.delegate(".deviceItem","click",function(e){
                var index=$(this).attr("data-index");
                self.ele.installDetails.html(template.render("tlpDetail",self.dataList[index]));
                self.ele.serviceInfo.hide();
                self.ele.installDetails.show();
            }).delegate(".scanBtn","click",function(e){
                self.ele.serviceInfo.show();
                self.ele.installDetails.hide();
            }).delegate(".receiptBtn","click",function(e){
                window.location.href="costDetails.html?customerId="+self.customerId+"&userId="+self.userId;
            });
