define(['jquery', 'tools','mobileScroll','bdTemplate'], function () {
    var page = {
        ele: {
            section: $("#section")
        },
	    ...
	    loadData:function(callback){
	    	...
	    	$.util.ajax({
	    		...
	    		data:{
	    			...
	    			PGToken:that.token,
	    			ReportAction: "ireport!teacherStatCoachReport.action",
	    		...
	    		success:function(data){
	    			...
	    			if(data.error.err_code==0){
						...
						for (var i = 0, len = list.length; i < len; i++) {
							...
							obj.coursecnt = item.coursecnt;
							arrayData.push(obj); 
						}
						
		    			//生成报表
						that.generateCharts(arrayData, startDate, endDate);
	    			}else{
	    ...
	    generateCharts: function (array, startDate, endDate) {
	        var that = this;
	        $("#content").html("");
	        for (var i = 0, il = array.length; i < il; i++) {
	            ...
	            var rowHtmlString = "<tr>" +
	                "<td>" + item.email + "</td>" +
	                "<td>" + item.name + "</td>" +
	                "<td><a href=\"" + "coachList.html?CustomerId=" + that.customerId +
