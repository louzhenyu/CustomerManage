    ...
    var page =
        {
            ...
            initTables: function () {
                ...
                this.loadData.args.Comment = comment;

                //分发起始日期
                var bindingBeginTime = $("#BindingBeginTime").val();
                this.loadData.args.BindingBeginTime = bindingBeginTime;

                //分发结束日期
                var bindingEndTime = $("#BindingEndTime").val();
                this.loadData.args.BindingEndTime = bindingEndTime;

                //操作人
                var operator = $("#Operator").val();
                this.loadData.args.Operator = operator;

                //会员名手机号等
                var vipCriteria = $("#VipCriteria").val();
                this.loadData.args.VipCriteria = vipCriteria;
            ...
            initEvent: function () {
                ...
                //导出按钮事件
                this.elems.exportBtn.click(function () {
                    //
                    //debugger;
                    var that = this;
                    $("#loading").show();
                    ...
                    //优惠券编号
                    var couponCode = $("#couponCode").val();
                    page.loadData.args.CouponCode = couponCode;

                    //组段号
                    var comment = $("#Comment").val();
                    page.loadData.args.Comment = comment;

                    //核销起始日期
                    var bindingBeginTime = $("#BindingBeginTime").val();
                    page.loadData.args.BindingBeginTime = bindingBeginTime;

                    //核销结束日期
                    var bindingEndTime = $("#BindingEndTime").val();
                    page.loadData.args.BindingEndTime = bindingEndTime;

                    //操作人
                    var operator = $("#Operator").val();
                    page.loadData.args.Operator = operator;

                    //初始化当前页为0
                    page.loadData.args.PageIndex = 0;

                    //请求结果   
                    page.loadData.exportExcel(function (data) {
                    });
        ...
        manageCouponSearch: function (callback) {
            ...
            $.util.ajax({
                ...
                data:
                {
                    ...
                    "BindingBeginTime": this.args.BindingBeginTime,
                    "BindingEndTime": this.args.BindingEndTime,
                    'VipCriteria': this.args.VipCriteria
                },
                success: function (data) {
        ...
        exportExcel: function (callback) {
            $.util.ajax({
                ...
                error: function (XMLResponse) {
                    //alert("您的导出文件保存在：" + XMLResponse.responseText); 
                    location.href = XMLResponse.responseText;
                }
            });
        },
        //根据提现ID获得入现列表
