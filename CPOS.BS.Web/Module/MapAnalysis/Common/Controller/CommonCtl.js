...
//    //CommonEn.aspx
//    window.location.href = "./CommonEn.aspx?mid=" + __mid;
...
function _map_Pop_Result(attributes, point) {
    ...
    pointInfo.storeUnitId = attributes.id;
    ...
    if (CurrentKPI.skpi_code.toUpperCase() == "EC44E43C-78BD-44C8-9ECA-30D031E27E12"
        ...
        || attributes.id == "1e8a94c1-167c-4b9c-a7db-9a9cf2915363".toLowerCase()
        || attributes.id == "5a287910-8754-4c4e-a222-f2fb5ba07d49".toLowerCase()
        )
    ) {
...
Ext.onReady(function () {
    //
    JITPage.HandlerUrl.setValue("Handler/CommonHandler.ashx?mid=" + "" + "&btnCode=search");
...
function startCPOSMonitoring() {
    Ext.Ajax.request({
        ...
        , callback: function (options, success, response) {
            ...
            if (success) {
                ...
                for (var i = 0; i < rst.UnitList.length; i++) {
                    //var point = findPointInfo("bd3a6de8-3c00-4ef6-b1c4-025d79bd1b8b".toLowerCase());
                    var storeIndex = i;
                    if (i >= rst.StoreList.length) storeIndex = 0;
                    var pointID =rst.StoreList[storeIndex].UnitId.toLowerCase();
                    //pointID='bd3a6de8-3c00-4ef6-b1c4-025d79bd1b8b';
                    var point = findPointInfo(pointID);
                    if (point == null)
                        return;
    ...
    //                if (difSeconds < 30) {//需要改变
                        ...
                        changedPoint.popurl = "http://o2oapi.aladingyidong.com/TerminalSalesInfo.aspx?unit_id=" + rst.UnitList[i].UnitId;
                ...
                if (dataes.length > 0) {
                    ...
                    mapObject._map_product_ChangePoint([]);
                }
...
function GetPoiData2(ax, by, Obj) {
    ...
    var unitId = "";
    if (z_rst != null && z_rst.StoreList != null) {
        for (var i = 0; i < z_rst.StoreList.length; i++) {
            if (z_rst.StoreList[i].UnitId == Obj.storeUnitId) {
                unitId = z_rst.UnitList[i].UnitId;
                break;
            }
        }
    }
    ...
    if (unitId == null || unitId.length == 0) {
        if (Obj.storeUnitId == "1e8a94c1-167c-4b9c-a7db-9a9cf2915363") {
            unitId = "9513b03e01d94dfd9e6313fbecc61f21";
        }
        else if (Obj.storeUnitId == "5a287910-8754-4c4e-a222-f2fb5ba07d49") {
            unitId = "10568493abfc4028b6c6d14de7dc5e0c";
        }
    }
    ...
    iString += '<iframe src="http://o2oapi.aladingyidong.com/TerminalSalesInfo.aspx?unit_id='+unitId+'" scroll="no" frameborder="0" width="500" height="450"></iframe></div></div><div class="clearFix_12"></div><div class="tip_leftArrow" style="top:175px;"></div></div>';
