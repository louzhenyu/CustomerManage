...
function changeKPI1(kpiMenuItem,typeid) {
    //var kpiID = kpiMenuItem.id;
    var kpi = findKPIBy(kpiMenuItem);
    // $("#GetPoiPop").remove();
    //
    if (kpi != null) {
        //设置筛选选项卡的可用性
        ...
        var mapObject = eval("window._map_getMapObject" + typeid + "()");
        ...
        //切换面<->气泡
        //        if (CurrentKPI) {
        //            if (CurrentKPI.skpi_code == k.skpi_code) {
        //                if (CurrentKPI.style_code == "SA") {
        //                    k.style_code = "SB";
        //                }
        //                else if (CurrentKPI.style_code == "SB") {
        //                    k.style_code = "SA";
        //                }
        //            }
        //        }
...
var _loadingMapAnalysis = false;
var _loadedMapCount = 0;
/*
根据点ID获取点信息
*/
...
获取KPI数据
*/
function _map_product_ReturnKpi_js(kpiCode, queryLayer, queryFilterLayer, queryFilterVal,typeid) {
    ...
    //
    var mask1 =null;
    if (!_loadingMapAnalysis) {
        mask1 = new Ext.LoadMask(Ext.getBody(), {
            msg: '加载数据,请等待...'
        });
    }
...
//            request.StyleCode ="SA";
    ...
    if (_cachedQueryLayer == _queryLayer && _cachedQueryFilterLayer == _queryFilterLayer && _cachedQueryFilterVal == _queryFilterVal && _cachedLayer[_queryLayer - 1] == _queryLayer + ',1'  && _strMapFlashCachedDataes != null && _strMapFlashCachedThresholds != null) {//使用缓存数据
        ...
        , callback: function (options, success, response) {
            _loadedMapCount++;
            if (_loadedMapCount >= 4) {
                _loadingMapAnalysis = false;
                mask1 = new Ext.LoadMask(Ext.getBody(), {
                    msg: '加载数据,请等待...'
                });
            }
