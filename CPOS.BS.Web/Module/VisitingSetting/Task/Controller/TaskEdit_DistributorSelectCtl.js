...
Ext.onReady(function () {
    ...
    JITPage.HandlerUrl.setValue("/Module/VisitingSetting/Task/Handler/TaskHandler_Distributor.ashx?mid=" + __mid);
...
function fnSubmit() {
    ...
    Ext.Ajax.request({
        url: JITPage.HandlerUrl.getValue()
                        + "&btncode=" + btncode + "&method=EditTaskPOP_Distributor",
        params: {
            id: id,
...
function fnAutoFillSet() {
    document.getElementById("tab1").setAttribute("src", "TaskEdit_DistributorAutoFillSet.aspx" + window.location.search);
