define(['jquery', 'template', 'tools'], function () {
    var page = {
        ...
        init: function () {
            ...
            this.UnitID = $.util.getUrlParam("UnitID");

            this.txtUnitCode = $("#txtUnitCode");
            this.txtUnitName = $("#txtUnitName");
            this.txtLeader = $("#txtLeader");
        ...
        ModifyDept: function (unitId, callback) {
            this.ajax({
                url: this.url,
                data: {
                    action: "ModifyDept",
        ...
        initEvent: function () {
            ...
            this.ele.section.delegate("#takeBtn", "click", function () {
                if (this.UnitID != '') {
                    self.ModifyDept(this.UnitID, function (data) {
                        alert(data.Message);
                    });
                } else {
                    var unitCode = "u1001";
                    self.AddDept(unitCode, function (data) {
                        alert(data.Message);
                    });
                }
