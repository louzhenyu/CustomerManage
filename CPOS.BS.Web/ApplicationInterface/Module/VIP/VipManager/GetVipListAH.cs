    ...
    public class GetVipListAH : BaseActionHandler<GetVipListRP, GetVipListRD>
    {
        protected override GetVipListRD ProcessRequest(DTO.Base.APIRequest<GetVipListRP> pRequest)
        {
            ...
            else
            {
                ...
                complexCondition.Add(new LikeCondition() { FieldName = "ul.path_unit_id", Value = "%"+loggingSessionInfo.CurrentUserRole.UnitId+"%" });
            }
            ...
            //调用会员卡管理列表查询
            var tempList = vipBLL.GetVipList(complexCondition.ToArray(), lstOrder.ToArray(), para.PageSize, para.PageIndex);
