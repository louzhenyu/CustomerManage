...
namespace JIT.CPOS.BS.DataAccess
{
    ...
    public partial class SysRetailRewardRuleDAO : Base.BaseCPOSDAO, ICRUDable<SysRetailRewardRuleEntity>, IQueryable<SysRetailRewardRuleEntity>
    {
        ...
        public DataSet GetItemListWithSharePrice(string strCustomerID, string strRetailTraderID, int intPageIndex, int intPageSize, string strSort, string strSortName)
        {
            ...
            strSql.Append("  END     ");
            strSql.AppendFormat(" ELSE BEGIN SELECT @ItemSalesPriceRate=ItemSalesPriceRate,@RetailTraderReward=RetailTraderReward FROM SysRetailRewardRule s WHERE IsTemplate=1  AND s.CooperateType='Sales'  AND s.RewardTypeCode='Sales' AND s.IsDelete=0 AND Status=1 AND s.CustomerId='{0}' ", strCustomerID);
            strSql.Append("  END     ");
            ...
            strSql.Append("  ,CAST(i.SalesPrice * @ItemSalesPriceRate*0.01 AS DECIMAL(18,2)) SharePrice ");
            strSql.Append("  ,CASE WHEN r.ItemId IS NULL THEN 0 ELSE 1 END  IsCheck"); strSql.Append("  ,(i.SalesPrice* (@ItemSalesPriceRate*0.01)*(@RetailTraderReward*0.01))  Commission, t.ifservice");
            strSql.Append("   FROM dbo.[vw_item_detail] i WITH(NOLOCK) ");
            strSql.AppendFormat(" LEFT JOIN RetailTraderItemMapping r WITH(NOLOCK) ON i.item_id=r.ItemId AND i.CustomerId=r.CustomerID and r.[IsDelete]=0   AND r.RetailTraderId= '{0}'", strRetailTraderID);
            strSql.Append(" LEFT JOIN dbo.T_Item t WITH ( NOLOCK ) ON r.ItemId = t.item_id ");
